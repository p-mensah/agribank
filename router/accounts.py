...
#     return f"GH{str(uuid.uuid4())[:8].upper()}"
...
#     if accounts_collection.find_one({"account_number": acc_num}):
#         raise HTTPException(500, "Account number collision")
...
#             raise HTTPException(400, "Minimum GH¢100 required")
#         account.update({"minimum_balance": 100.0, "interest_rate": 3.5})
...
#             raise HTTPException(400, "Tenure required")
#         from dateutil.relativedelta import relativedelta
...
#     accounts_collection.update_one({"_id": id}, {"$set": {"status": "CLOSED"}})
...
#     return {"interest_earned": interest, "new_balance": new_balance}

# @router.patch("/{id}/freeze")
# def freeze_account(id: str, freeze: bool, user=Depends(authenticated_user)):
#     acc = accounts_collection.find_one({"_id": id, "user_id": user["_id"]})
#     if not acc:
#         raise HTTPException(404, "Account not found")
#     status = "FROZEN" if freeze else "ACTIVE"
#     accounts_collection.update_one({"_id": id}, {"$set": {"status": status}})
